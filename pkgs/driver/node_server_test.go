...
const (
	...
	TestSocketFilename string = "unix:///tmp/csi-tkm/notused.sock"

	// TestImagePort is the location of port the Image Server will listen on for TKM
	// to send requests.
	TestImagePort string = ":50051"

	// TestCacheDir is the default root directory to store the expanded the Triton Kernel
	// images.
	TestCacheDir = "/tmp/csi-tkm/caches"
)
...
func TestNodePublishVolume(t *testing.T) {
	t.Run("Publish a volume", func(t *testing.T) {
		...
		log := ctrl.Log.WithName("tkm-csi-driver")

		nodeName := "worker-1"
		tkmNamespace := "tkm-system"

		// Get a Driver instance that will be tested.
		d, err := NewDriver(log, nodeName, tkmNamespace, TestSocketFilename, TestCacheDir, true)
		...
		podName1 := "testPod-1"
		podNs1 := "testpod1"
		tkmCacheCrd1 := "rocmKernel"

		// Instance 2: Namespace Scoped
		tkmCacheNs2 := "testpod2"
		...
		podName2 := "testPod-2"
		podNs2 := "testpod2"
		tkmCacheCrd2 := "rocmKernel"

		// Instance 1 - Populated the Request Structure for a NodePublishVolume() call.
		pubReq1, err := buildNodePublishVolumeRequest(volumeId1, podUid1, podName1, podNs1, tkmCacheCrd1)
		require.NoError(t, err)
		...
		err = createImageFiles(TestCacheDir, tkmCacheNs1, tkmCacheCrd1)
		require.NoError(t, err)
		...
		pubReq2, err := buildNodePublishVolumeRequest(volumeId2, podUid2, podName2, podNs2, tkmCacheCrd2)
		require.NoError(t, err)

		// Instance 2 - Create files from the OCI Image download
		err = createImageFiles(TestCacheDir, tkmCacheNs2, tkmCacheCrd2)
		require.NoError(t, err)
...
func buildNodePublishVolumeRequest(volumeId, podUid, podName, podNs, tkmCacheCrd string,
	...
	if tkmCacheCrd != "" {
		req.VolumeContext["csi.tkm.io/TKMCache"] = tkmCacheCrd
	}
...
func cleanUp(testDir string, d *Driver, log logr.Logger) error {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			...
			// /tmp/csi-tkm/kubelet/pods/fedcba98-7654-3210-dead-000000000001/volumes/kubernetes.io~csi/kernel-volume/mount
			if name == "kubelet" {
