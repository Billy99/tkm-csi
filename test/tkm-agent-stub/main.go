...
func main() {
	...
	flag.BoolVar(&loadFlag, "load", false,
		"Download and unpack and OCI Image containing a Triton Kernel Cache and\n"+
			"load the cached kernel on the local filesystem.\n")

	flag.BoolVar(&unloadFlag, "unload", false,
		"Remove an already downloaded OCI Image containing a Triton Kernel Cache\n"+
			"from the local filesystem\n")

	flag.StringVar(&image, "image", "",
		"Image repository URL of OCI Image with Triton Kernel Cache.\n"+
			"Example: -image quay.io/repository/mtahhan/01-vector-add-cache:latest\n")

	flag.StringVar(&crdName, "crdName", "",
		"Name of the Triton Kernel Cache CRD instance. If testing in a non-Kubernetes\n"+
			"environment, this is just a tag. Same value from \"csi.tkm.io/tritonKernelCache\"\n"+
			"in the PodSpec Volume.\n"+
			"Example: -crdName kernel-x\n")

	flag.StringVar(&namespace, "namespace", "",
		"Namespace of the Triton Kernel Cache CRD instance. Leave blank for cluster scoped CRDS.\n"+
